...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // 3. Add token as a dependency to re-run if it changes
    ...
    if (id && token) {
      ...
      const fetchAllData = async () => {
        ...
        try {
          ...
          const analysisUrl = `http://localhost:3001/resumes/${id}`;

          const matchesUrl = `http://localhost:3001/resumes/${id}/matches`;



          // 4. THIS IS THE FIX: Add the Authorization header to both requests
  ...
  }, [id, token]); // Dependency array updated
